...
bool TFT_eSprite::pushRotated(int16_t angle, uint32_t transp) {
  ...
  for (int32_t y = min_y; y <= max_y; y++, yt++) {
    ...
    do {
      ...
      if (_bpp == 16) { rp = _img[xp + yp * _iwidth]; } else {
        rp = readPixel(xp, yp);
...
bool TFT_eSprite::pushSprite(int32_t tx, int32_t ty, int32_t sx, int32_t sy, int32_t sw, int32_t sh) {
  ...
  */
  ...
  if (_bpp == 16) {
    ...
    // Check if a faster block copy to screen is possible
    if (sx == 0 && sw == _dwidth) _tft->pushImage(tx, ty, sw, sh, _img8 + _iwidth * _ys, (bool)true);
    else // Render line by line
      while (sh--) _tft->pushImage(tx, ty++, sw, 1, _img8 + _xs + _iwidth * _ys++, (bool)true);
  } else if (_bpp == 4) {
    // Check if a faster block copy to screen is possible
  ...
  {
    // Check if a faster block copy to screen is possible
    if (sx == 0 && sw == _dwidth) _tft->pushImage(tx, ty, sw, sh, _img8 + (_bitwidth >> 3) * _ys, (bool)false);
    else // Render line by line
    {
      _tft->startWrite();
      while (sh--) {
...
uint16_t TFT_eSprite::readPixelValue(int32_t x, int32_t y) {
  ...
  if (_bpp == 4) {
    ...
    if ((x & 0x01) == 0) return _img4[((x + y * _iwidth) >> 1)] >> 4; // even index = bits 7 .. 4
    else return _img4[((x + y * _iwidth) >> 1)] & 0x0F; // odd index = bits 3 .. 0.
  }
