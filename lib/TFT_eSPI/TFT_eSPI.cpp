...
void TFT_eSPI::drawXBitmap(int16_t x, int16_t y, const uint8_t* bitmap, int16_t w, int16_t h, uint16_t color, uint16_t bgcolor) {
  ...
  for (j = 0; j < h; j++) {
    for (i = 0; i < w; i++) {
      if (pgm_read_byte(bitmap + j * byteWidth + i / 8) & (1 << (i & 7))) drawPixel(x + i, y + j, color);
      else drawPixel(x + i, y + j, bgcolor);
    }
  }
...
int16_t TFT_eSPI::textWidth(const char* string, uint8_t font) {
  ...
  if (font > 1 && font < 9) {
    ...
    while (*string) {
      uniCode = *(string++);
      if (uniCode > 31 && uniCode < 128) str_width += pgm_read_byte(widthtable + uniCode); // Normally we need to subtract 32 from uniCode
  ...
  if (!gfxFont) { // 'Classic' built-in GLCD font
    ...
    if ((size == 1) && fillbg && !clip) {
      ...
      for (int8_t i = 0; i < 6; i++) {
        uint8_t line;
        if (i == 5) line = 0x0;
        else line = pgm_read_byte(&font[0] + (c * 5) + i);
...
void TFT_eSPI::drawThickLine(int32_t x0, int32_t y0, int32_t x1, int32_t y1, uint32_t color, float wd) {
  if (_vpOoB) return;
  inTransaction = true;
  int dx = abs(x1 - x0), sx = x0 < x1 ? 1 : -1;
  int dy = abs(y1 - y0), sy = y0 < y1 ? 1 : -1;
  int err = dx - dy, e2, x2, y2; /* error value e_xy */
  float ed = dx + dy == 0 ? 1 : sqrt(static_cast<float>(dx) * dx + static_cast<float>(dy) * dy);

  for (wd = (wd + 1) / 2; ;) { /* pixel loop */
    drawPixel(x0, y0, color, 255 - max(0, static_cast<int>(255 * (abs(err - dx + dy) / ed - wd + 1))), 0x00FFFFFF);
    e2 = err;
    x2 = x0;
    if (2 * e2 >= -dx) { /* x step */
      for (e2 += dy, y2 = y0; e2 < ed * wd && (y1 != y2 || dx > dy); e2 += dx) drawPixel(x0, y2 += sy, color, 255 - max(0, static_cast<int>(255 * (abs(e2) / ed - wd + 1))), 0x00FFFFFF);
      if (x0 == x1) break;
      e2 = err;
      err -= dy;
      x0 += sx;
    ...
    if (2 * e2 <= dy) { /* y step */
      for (e2 = dx - e2; e2 < ed * wd && (x1 != x2 || dx < dy); e2 += dy) drawPixel(x2 += sx, y0, color, 255 - max(0, static_cast<int>(255 * (abs(e2) / ed - wd + 1))), 0x00FFFFFF);
      if (y0 == y1) break;
      err += dx;
      y0 += sy;
    }
...
void TFT_eSPI::setAttribute(uint8_t attr_id, uint8_t param) {
  switch (attr_id) {
    break;
  case CP437_SWITCH: _cp437 = param;
    break;
  case UTF8_SWITCH: _utf8 = param;
    decoderState = 0;
    break;
  case PSRAM_ENABLE:
#if defined (ESP32) && defined (CONFIG_SPIRAM_SUPPORT)
...
uint8_t TFT_eSPI::getAttribute(uint8_t attr_id) {
  ...
  case UTF8_SWITCH: // ON/OFF control of UTF-8 decoding
    return _utf8;
  case PSRAM_ENABLE: return _psram_enable;
...
int16_t TFT_eSPI::drawString(const char* string, int32_t poX, int32_t poY, uint8_t font) {
  ...
  if (textdatum || padX) {
    switch (textdatum) {
    case TC_DATUM: poX -= cwidth / 2;
      padding += 1;
      break;
    case TR_DATUM: poX -= cwidth;
      padding += 2;
      break;
    case ML_DATUM: poY -= cheight / 2;
    //padding += 0;
      break;
    case MC_DATUM: poX -= cwidth / 2;
      poY -= cheight / 2;
      padding += 1;
      break;
    case MR_DATUM: poX -= cwidth;
      poY -= cheight / 2;
      padding += 2;
      break;
    case BL_DATUM: poY -= cheight;
    //padding += 0;
      break;
    case BC_DATUM: poX -= cwidth / 2;
      poY -= cheight;
      padding += 1;
      break;
    case BR_DATUM: poX -= cwidth;
      poY -= cheight;
      padding += 2;
      break;
    case L_BASELINE: poY -= baseline;
    //padding += 0;
      break;
...
#endif
    switch (padding) {
    case 1: fillRect(padXc, poY, padX - cwidth, cheight, textbgcolor);
      break;
    case 2: fillRect(padXc, poY, (padX - cwidth) >> 1, cheight, textbgcolor);
      padXc = poX - ((padX - cwidth) >> 1);
      fillRect(padXc, poY, (padX - cwidth) >> 1, cheight, textbgcolor);
      break;
    case 3: if (padXc > padX) padXc = padX;
